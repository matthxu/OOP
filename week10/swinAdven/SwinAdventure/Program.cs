...
namespace MainProgram
{
    ...
    class MainClass
    {
        public static void Main(string[] args)
        {
            ...
            Item donut = new(new List<string> { "food", "donut" }, "Donut", "A glazed donut");
            Item beanie = new(new List<string> { "hat", "clothes" }, "Beanie", "A warm knitted beanie");

            newPlayer.Inventory.Put(donut);
            newPlayer.Inventory.Put(beanie);

            Bag rucksack = new(new List<string> { "bag", "rucksack" }, "Rucksack", "A large rucksack");
            newPlayer.Inventory.Put(rucksack);

            Item persimmon = new(new List<string> { "food", "fruit" }, "Persimmon", "A ripe persimmon");
            rucksack.Inventory.Put(persimmon);

            bool finished = false;
            LookCommand cmd = new LookCommand(new List<string> { "look" });
            while (!finished)
            {
